...
function main($, _) {
	...
	var numMinions = count(initialSet, true);
        ...
        var round1Results = $analyzeTBody.children('tr').map(function(idx, e) {
            return $($(e).find('input')[2]).is(':checked');
        });
        var round2Results = $analyzeTBody.children('tr').map(function(idx, e) {
	...
	if (_.some(minionResults)) {
        ...
        console.log(applicableSets);
        if (_.contains(round1Results, true)) {
            applicableSets = _.filter(applicableSets, isSetPossible, round1Results);
        }
        console.log('possibilities after first cull');
        console.log(applicableSets);
        if (_.contains(round2Results, true)) {
            applicableSets = _.filter(applicableSets, isSetPossible, round2Results);
        }
       ...
       //var maxProbability = _.reduce(relativeProbabilities, function(memo, prob) {
       //  return Math.max(memo, prob);}, 0);
